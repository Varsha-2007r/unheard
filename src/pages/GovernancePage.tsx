  ...
  const privacyPillars = [
    {
      ...
      desc: "Zero retention of personal IP addresses, device identifiers, or biometric tracking cookies at the edge."
    },
    {
    ...
    <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {privacyPillars.map((p, idx) => (
          <div
            key={idx}
            className="p-6 rounded-3xl bg-white border border-[#D9E2F0] hover:border-[#2563EB] hover:shadow-md transition-all space-y-3 group shadow-xs"
          >
      ...
      <div className="p-7 sm:p-9 rounded-3xl bg-white border border-[#D9E2F0] space-y-6 shadow-md">
        <div className="flex items-center space-x-2.5 pb-3 border-b border-[#D9E2F0]">
          ...
          <h2 className="text-base sm:text-lg font-display font-bold text-[#0F172A] uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-mono">
          ...
          <div className="space-y-3 p-5 rounded-2xl bg-[#DBEAFE]/30 border border-[#BFDBFE]">
            <div className="text-[#1D4ED8] font-bold uppercase flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-[#2563EB]" />
              <span>WHAT UNHEARD DELIVERS:</span>
            </div>
            <ul className="space-y-2 text-[#0F172A] list-disc pl-4 leading-relaxed font-sans text-xs">
              <li>Early-warning indicators derived from empirical qualitative volume surges.</li>
              <li>Actionable prioritization briefs for institutional infrastructure allocation.</li>
              <li>Privacy-first citizen listening without identity leakage or exposure.</li>
              <li>Continuous correlation across municipal and educational sectors.</li>
            </ul>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-[#F8FAFC] border border-[#D9E2F0] text-center text-xs font-mono text-[#64748B]">
          UNHEARD is an AI-powered social-impact decision support system. Humans remain ultimately responsible for policy execution and verification.
        </div>
      </div>
